...
var JSCCommon = {
	...
	LazyFunction: function LazyFunction() {
		// Для лэзи загрузки 
		// лэзи 
		...
		document.addEventListener("DOMContentLoaded", function () {
			var lazyImages = [].slice.call(document.querySelectorAll("video.lazy"));
			var active = false;
					...
					setTimeout(function () {
						lazyImages.forEach(function (lazyImage) {
							if (lazyImage.getBoundingClientRect().top - lazyImage.parentElement.clientHeight * 4 <= window.innerHeight && lazyImage.getBoundingClientRect().bottom + lazyImage.parentElement.clientHeight * 4 >= 0 && getComputedStyle(lazyImage).display !== "none") {
								lazyImage.poster = lazyImage.dataset.poster; // lazyImage.srcset = lazyImage.dataset.srcset;
								// lazyImage.classList.remove("lazy");
	...
	tabscostume: function tabscostume(tab) {
		...
		$('.' + tab + '__caption').on('click', '.' + tab + '__btn:not(.active)', function (e) {
			$(this).addClass('active').siblings().removeClass('active').closest('.' + tab).find('.' + tab + '__content').hide().removeClass('active').eq($(this).index()).addClass('active').fadeIn(function () {
				...
				slider.hasClass("swiper-container-initialized") ? mySwiper5.update() : mySwiper5 = new Swiper(slider, params);
			});
		}); // }
	...
	inputMask: function inputMask() {
		// mask for input
		$('input[type="tel"]').attr("pattern", "[+][0-9]{1}[(][0-9]{3}[)][0-9]{3}-[0-9]{2}-[0-9]{2}").inputmask("+9(999)999-99-99"); // mask for input
	...
	stickyFunc: function stickyFunc() {
		if (window.matchMedia("(min-width: 1200px)").matches) {
			$(".aside-nav").stick_in_parent({
				...
				parent: '.main-row' // // recalc_every: 1,
				//  recalc_every: true,

			});
		}
	},
	asideNav: function asideNav() {
		$('.toggle--js, .aside-nav__link--js').click(function () {
			var th = $(this);
			th.parent().find('.toggle--js').toggleClass('active').next('.aside-nav__sub').slideToggle(function () {
				$(this).toggleClass('aside-nav__sub--open');
			}).parent().siblings().find('.toggle--js').removeClass('active').next('.aside-nav__sub').slideUp(function () {
...
jQuery(document).ready(function ($) {
	...
	JSCCommon.asideNav();
	JSCCommon.accordion();
	JSCCommon.videoBg(); // / закрыть меню при горизонтальном свайпе
